...
public class MyPagerAdapterNoRotate extends FragmentPagerAdapter implements 
ViewPager.OnPageChangeListener {
	...
	private boolean mSliderMove = false;
	...
	public MyPagerAdapterNoRotate(Context context, FragmentManager fm) {
		
		super(fm);
		Log.i("hm", "create MyPagerAdapterNoRotate");
		this.fm = fm;
	...
	public void updateData(Integer id){
		mSliderData.clear();
		for (int i = 0; i < ((MainControlBarActivity)context).mSliderData.size(); i++) {
			if(((MainControlBarActivity)context).mSliderData.get(i).cat_id == 0 
					|| ((MainControlBarActivity)context).mSliderData.get(i).cat_id == id){
				mSliderData.add(((MainControlBarActivity)context).mSliderData.get(i));
			}
		}
		SliderData data = new SliderData();
		mSliderData.add(0, data);
		mSliderData.add(data);
		((MainControlBarActivity)context).setSliderCount(mSliderData.size());
