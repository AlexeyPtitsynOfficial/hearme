...
{	
	...
	private class AccessTokenActionsTask extends AsyncTask<View, Void, String> {
	    ...
	    protected String doInBackground(View... params) {
	    	...
	    	new CustomToast(getActivity()).showErrorToast("Тип аккаунта: "+String.valueOf(social_type));
	    	new CustomToast(getActivity()).showErrorToast("позиция: "+String.valueOf(acc_pos));
	    	return null;
	    }
	    ...
	    protected void onPostExecute(String access_token) {
	        if (access_token != null) {
				...
				new CustomToast(getActivity()).showErrorToast("Сессия устарела");
				return;
	        }
	    }
																									...
																									GetJSONListener
	{
		...
		public View getTitleView(int position, View convertView,
			...
			if (convertView == null) {
				...
				viewHolder.imageView.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						// TODO Auto-generated method stub
						GeneralUserData data = (GeneralUserData)v.getTag();
			...
			viewHolder.imageView.setTag(data);
			...
			if(mMainControlBarActivity.mAdListData.get(position).user_image_url != null)
				...
				viewHolder.imageView.setImageResource(R.drawable.icon_no_avatar);
		...
		private class ContentViewHolder
		{
			...
			RelativeLayout btn_add_comment;
		}
		...
		public View getContentView(int position, View convertView,
				...
				if(convertView == null)
				{
					...
					viewHolder.comment_text.setOnFocusChangeListener(new View.OnFocusChangeListener() {
						
						@Override
						public void onFocusChange(View v, boolean hasFocus) {
							// TODO Auto-generated method stub
							RelativeLayout fav_ad = (RelativeLayout)v.getTag(R.string.fav_ad);
							RelativeLayout fav_user = (RelativeLayout)v.getTag(R.string.fav_user);
							if(hasFocus && fav_ad.getVisibility() == View.VISIBLE)
							{
								fav_ad.setVisibility(View.GONE);
								fav_user.setVisibility(View.GONE);
							}
							else if(!hasFocus && fav_ad.getVisibility() == View.GONE)
							{
					...
					viewHolder.btn_add_comment = (RelativeLayout)convertView.findViewById(R.id.btn_add_comment);
				...
				viewHolder.comment_text.setTag(R.string.fav_ad, viewHolder.btn_fav_ad);
		...
		private class ContentCreateTask extends AsyncTask<Integer, Void, Void>{
			...
			protected void onPostExecute(Void result) {
				...
				viewHolder.btn_fav_ad.setTag(mMainControlBarActivity.mAdListData.get(mPos).Id);
				
				viewHolder.mFavorite_user.setTag(mMainControlBarActivity.mAdListData.get(mPos).user_id);
		...
		public void onClick(View v) {
			...
			if(mMainControlBarActivity.getApiClient().isConnected() || mMainControlBarActivity.isLogin()){
				...
				case R.id.btn_add_comment:
					...
					String str = comment_text.getText().toString().trim();
					if(str.length() == 0){
						new CustomToast(getActivity()).show(getString(R.string.hint_text));
						return;
					}
					ImageView comment_btn = (ImageView)v.findViewById(R.id.comment_btn_image);
					ProgressBar comment_progress = (ProgressBar)v.findViewById(R.id.comment_add_progress);
		...
		public void clickAction(View v, String url, String access_token){
			...
			case R.id.btn_add_comment:
				...
				params.put("ad_id", mMainControlBarActivity.mAdListData.get(pos).Id.toString());
				params.put("comment_text", mEncodeCommentText);
		...
		public void onRemoteCallComplete(JSONObject obj, View v) {
			...
			try {
				...
				// TODO Auto-generated catch block
				new CustomToast(getActivity()).showErrorToast("Ошибка");
				e.printStackTrace();
			}
