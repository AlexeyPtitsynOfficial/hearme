																		...
																		OnConnectionFailedListener
{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mImageThumbSize = getResources().getDimensionPixelSize(R.dimen.image_slider_thumb_size);
        ...
        if(mImageFetcherBack == null)
        	mImageFetcherBack = new ImageFetcherBack(this, mImageThumbSize);
        mImageFetcherBack.setLoadingImage(R.drawable.slider_load);
	...
	public void showFilterStateInfo(boolean show){
		...
		if(mCatIDsList.size() != 0){
			mFilterArray.add(getString(R.string.filter_category).toUpperCase());
			filter_not_exist = false;
		}
		if(mSelectedAdsId.size() != 0){
			mFilterArray.add(getString(R.string.filter_fav_ads).toUpperCase());
			filter_not_exist = false;
		}
		if(getCurrentMenuType() == DrawerListAdapter.AD_MY){
			mFilterArray.add(getString(R.string.my_ad).toUpperCase());
			filter_not_exist = false;
		}
			...
			mFilterArray.add(getString(R.string.filter_fav_users).toUpperCase());
			filter_not_exist = false;
		}
		if(mMainSearchText != null){
			mFilterArray.add(getString(R.string.filter_search).toUpperCase());
			filter_not_exist = false;
		}
	...
	public void createCatalogItems()
	{
		...
		TreeElement el = new TreeElement("-1",
				"all",
				getString(R.string.all_cats), 
			false, 
			1, null, 0, false);
		mTreeElements.add(0,el);
	...
	public ListView getCatDrawerList(){
		return mCatDrawerList;
	}
	
	public void createCatTree(CategoryData data){
		
		if(data == null)
			return;
		
		TreeElement el = new TreeElement(data.Id.toString(),data.icon_img, data.Name, false, data.hasChild, null, 0, false);
		mTreeElements.add(el);
		if(data.hasChild == 1){
			createCatTree(data);
		}
	}
	...
	public void getMainParentId(Integer pos){
		if(mCategoryData.size() <pos)
			return;
		if(mCategoryData.get(pos).ParentId.equals(0)){
			mSelectedMainParentId = mCategoryData.get(pos).Id;
			return;
		}
		
		for (int i = 0; i < mCategoryData.size(); i++) {
			if(mCategoryData.get(pos).ParentId.equals(mCategoryData.get(i).Id)){
				getMainParentId(i);
				break;
			}
		}
	...
	private void createMenuItems(){
		...
		{			
	        ...
	        mItems.add(new DrawerListItems(MainControlBarActivity.this, inflater, getResources().getDrawable(R.drawable.icon_fav_user), R.drawable.icon_fav_user, getString(R.string.ad_favorite_users), DrawerListAdapter.AD_FAVORITE_USERS));
	        mItems.add(new DrawerListItems(MainControlBarActivity.this, inflater, getResources().getDrawable(R.drawable.icon_fav_ad), R.drawable.icon_fav_ad, getString(R.string.ad_favorite_ads), DrawerListAdapter.AD_FAVORITE));
		}
		else
		{
        ...
        mItems.add(new DrawerListItems(MainControlBarActivity.this, inflater, getResources().getDrawable(R.drawable.icon_report), R.drawable.icon_report, getString(R.string.add_report), DrawerListAdapter.REPORT));
        ...
        if(mMenuFragment != null /*&& mMenuFragment.getAdapter() != null*/){
        	mMenuFragment.setData(mItems);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		String title = "HearMe";
		String desc = "Приглашаю присоединиться к нашему обществу - http://play.google.com";
	...
	public void selectItem(int type, int secondtype) 
	{
		...
		case DrawerListAdapter.AD_ALL:
			...
			if(mCatIDsList.size() != 0){
				showFilterStateInfo(true);
				mSavedData.saveSelectedCats(mCatIDsList);
			}
			else
				showFilterStateInfo(false);
		...
		case DrawerListAdapter.TELL_FRIENDS:
			...
			shareIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "HearMe");
			shareIntent.putExtra(android.content.Intent.EXTRA_TEXT, desc);
			mShareActionProvider.setShareIntent(shareIntent);
		...
		if( data == null)
		{
			if (requestCode == REGISER_USER) {
				...
				if(resultCode == ProfileActivity.LOG_OUT){
					...
					mConnectionProgressDialog.setMessage("Выход из профиля");
					mConnectionProgressDialog.show();
				}
	...
	public void getDownloadDCompleteState(int loadType, ArrayList<? extends Object> data_array) {
		...
		if(mSavedData.isFirstAppRun())
			mStartAppFragment.setLoadInfo("Настройка программы");
		ArrayList<String> checkConn = (ArrayList<String>)data_array;
		if(checkConn.size() != 0)
			if(checkConn.get(0) == "no_connection"){
				if(mSavedData.isFirstAppRun()){
